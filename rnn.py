...
class RNN(nn.Module):
    ...
    def forward(self, inputs):
        batch_size = inputs.size(1)  # Get batch size from inputs
        h0 = torch.zeros(self.numOfLayer, batch_size, self.h)  # Adjust h0 based on the batch size and hidden dimension
        out, hidden = self.rnn(inputs, h0)
        out = self.W(out[-1])  # Only take the last output if you’re looking for the final prediction
